...
if ITER_TRAIN_MATRIX:
    ...
    for i in range(len(traj)):
        ...
        # 保存编码器和解码器
        torch.save(model.regressor._regressor._encoder.state_dict(), f"{model_save_dir}\\encoder_sand_traj_{i}.pth")
        torch.save(model.regressor._regressor._decoder.state_dict(), f"{model_save_dir}\\decoder_sand_traj_{i}.pth")
            ...
            for k in range(5):
                ...
                for t in range(validation_num):
                    if traj_val.shape[0] > index_start_step + t + 1:
                        ...
                        pxT = torch.tensor(traj_val[index_start_step + t, :n].reshape((1, n)), dtype=torch.float32)
                        uT = torch.tensor(traj_val[index_start_step + t, n:n+m].reshape((1, m)), dtype=torch.float32)
                        pred = model.predict(x=pxT, u=uT).reshape(n)
