...
from .regression import NNDMDc
from .regression import NNDMDcm
...
class Koopman(BaseEstimator):
    ...
    def predict(self, x, u=None):
        ...
        print("@@@@@@@@@@@@@@@@@ using the predict function in koopman @@@@@@@@@@@@@@@@@ ")
        check_is_fitted(self, "n_output_features_")
        x.requires_grad_()
        # here x is self.rank and u is self.m, right size!
    ...
    def _step(self, x, u=None):
        ...
        if u is None or self.n_control_features_ == 0:
            if self.n_control_features_ > 0:
                ...
                ### 有报错
                # if isinstance(self.regressor, NNDMDc):
                #     x = np.concatenate((x, u), axis=1)
                # if isinstance(self.regressor, NNDMDcm):
                #     x = np.concatenate((x, u), axis=1)
                ### 修改版，使用tensor实现
                x = torch.tensor(x) if isinstance(x, np.ndarray) else x
                u = torch.tensor(u) if isinstance(u, np.ndarray) else u
                if isinstance(self.regressor, NNDMDc):
                    x = torch.cat((x, u), dim=1)
                elif isinstance(self.regressor, NNDMDcm):
                    x = torch.cat((x, u), dim=1)


            # print("######################################################")
            # print("what is x in predict?", x.shape)

            return self.model.predict(X=x)
        else:
            if not isinstance(self.regressor, DMDc) and not isinstance(self.regressor, EDMDc) and not isinstance(self.regressor, NNDMDc) and not isinstance(self.regressor, NNDMDcm):
                raise ValueError(
                    "Control input u was passed, but self.regressor is not DMDc or EDMDc or NNDMDc or NNDMDcm"
                )
    ...
    def state_transition_matrix(self):
        ...
        if not isinstance(self.regressor, DMDc) and not isinstance(self.regressor, EDMDc):
            raise ValueError(
                "self.regressor is not DMDc and EDMDc, so object has no "
    ...
    def control_matrix(self):
        ...
        if not isinstance(self.regressor, DMDc) and not isinstance(self.regressor, EDMDc):
            raise ValueError(
                "self.regressor is not DMDc and EDMDc, so object has no control_matrix"
            )
    ...
    def get_Koopman_Operator(self):
        A_discrete = self.regressor.A
        state_matrix = self.regressor.K_state_
        action_matrix = self.regressor.K_action_
        K = self.regressor.K_
        return A_discrete, state_matrix, action_matrix, K


    def save_model(self, model_dir):
        if hasattr(self.regressor, "save_model"):
            self.regressor.save_model(model_dir)
        else:
            raise NotImplementedError("Regressor does not support save_model.")

    def load_encoder_weights(self, encoder_path):
        if hasattr(self.regressor, "load_encoder_weights"):
            self.regressor.load_encoder_weights(encoder_path)
        else:
            raise NotImplementedError("Regressor does not support load_encoder_weights.")
    
    def load_koopman_matrix(self, K_state_path, K_action_path):
        if hasattr(self.regressor, "load_koopman_matrix"):
            self.regressor.load_koopman_matrix(K_state_path, K_action_path)
        else:
            raise NotImplementedError("Regressor does not support load_koopman_matrix.")

    def freeze_encoder(self):
        if hasattr(self.regressor, "freeze_encoder"):
            self.regressor.freeze_encoder()
        else:
            raise NotImplementedError("Regressor does not support freeze_encoder.")

    def freeze_matrix(self):
        if hasattr(self.regressor, "freeze_matrix"):
            self.regressor.freeze_matrix()
        else:
            raise NotImplementedError("Regressor does not support freeze_matrix.")
