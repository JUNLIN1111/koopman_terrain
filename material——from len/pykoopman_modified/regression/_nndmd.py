...
class StandardKoopmanOperator(BaseKoopmanOperator):
    ...
    def __init__(self, **kwargs):
        ...
        # 初始化self.K，这是一个dim*dim大小的对角矩阵，使用trunc_normal_截断正态分布进行初始化填充
...
class DLKoopmanRegressor(L.LightningModule):
    ...
    def __init__(
        ...
        # print("### ### ### ### ### ### ### ###")
        # print("encoder input_size = ", config_encoder["input_size"])
        # print("encoder output_size = ", config_encoder["output_size"])
        # print("### ### ### ### ### ### ### ###")
        ...
        # print("### ### ### ### ### ### ### ###")
        # print("decoder input_size = ", config_decoder["input_size"])
        # print("decoder output_size = ", config_decoder["output_size"])
        # print("### ### ### ### ### ### ### ###")


        # Koopman Operator的操作符，其决定了在高维空间中如何演化
        if mode == "Dissipative":
            ...
            def training_step(batch, batch_idx):
                ...
                def closure():
                    ...
                    decoded_y_seq_rnn = torch.zeros(
                        (x.size(0), self.look_forward, self.encoder_input_size),
                        device=self.device,
                    )
    ...
    def forward(self, x, n=1):
        ...
        """
        encoded = self._encoder(x)  # 升维
        phi_seq = self._propagate_encoded_n_steps(encoded, n)   # 在升维空间中演化
        decoded = self._decoder(phi_seq[:, -1, :])  #降维
        return decoded
    ...
    def training_step(self, batch, batch_idx):  # 计算训练中每一步的loss值
        ...
        ### 这里的input size是27（x+u的维度）
        ### standard RNN loss
        # decoded_y_seq_rnn = torch.zeros(
        #     (x.size(0), self.look_forward, self.encoder_input_size), device=self.device
        # )
        decoded_y_seq_rnn = torch.zeros(
        ...
        for i in range(batch_look_forward):
            # print("####################################")
            # print("phi_seq size = ", phi_seq.shape)
            # print("phi_seq after decoder size = ", self._decoder(phi_seq[:, i, :]).shape)
            # print("decoded_y_seq_rnn size = ", decoded_y_seq_rnn.shape)
            # print("####################################")
            decoded_y_seq_rnn[:, i, :] = self._decoder(phi_seq[:, i, :])
        rnn_loss = self.masked_loss_metric(decoded_y_seq_rnn, y, ys)
        ...
        # autoencoder reconstruction loss
        # for x
        decoded_x = self._decoder(encoded_x)    # decoded_x (91, 21), x (91, 27)
        # print("###################")
        # print("decoded_x size = ", decoded_x.shape)
        # print("x size = ", x.shape)
        # print("##################")
        # 处理维度不一致
        if decoded_x.size(1) < x.size(1):
            x_x = torch.zeros_like(decoded_x)
            x_x[:, :] = x[:, :decoded_x.size(1)]
        rec_loss = torch.nn.functional.mse_loss(decoded_x, x_x)
