...
class MaskedMSELoss(nn.Module):
    ...
    def forward(self, output, target, target_lens):
        ...
        Returns:
            The MSE loss as a scalar tensor.
        """
        if CHECK_FORWARD_FUNCTION:
            print("###### forward function in MaskedMSELoss")
        ...
        # 若target和output向量长度不匹配，则比较前面的位数
        if target.size(2) > output.size(2):
            target_x = torch.zeros_like(output)
            target_x[:, :, :] = target[:, :, :output.size(2)]
        ...
        squared_diff = torch.pow(output - target_x, 2)
...
class FFNN(nn.Module):
    ...
    def __init__(
        self, input_size, hidden_sizes, output_size, activations, include_state=False, requires_grad=False
    ):
    ...
    def forward(self, x):
        ...
        if CHECK_FORWARD_FUNCTION:
            print("###### forward function in FFNN")

        # print("x.requires_grad:", x.requires_grad)
        # print("Iutput grad_fn:", x.grad_fn)
        ...
        if CHECK_GRAD:
            print(f"Output in FFNN: x requires_grad: {x.requires_grad}, grad_fn: {x.grad_fn}")
...
class BaseKoopmanOperator_with_control(nn.Module):
    ...
    def forward(self, x):
        """
        Computes the forward pass, input x = [state, u]
        """
        if CHECK_FORWARD_FUNCTION:
            print("###### forward function in BaseKoopmanOperator_with_control")
        ...
        koopman_operator = self.get_K()
        state = x[:, :self.dim]
        action = x[:, self.dim:]
        xnext = torch.matmul(x, koopman_operator.t())
        if CHECK_GRAD:
            print(f"Output in Base KO with c: xnext requires_grad: {xnext.requires_grad}, grad_fn: {xnext.grad_fn}")
        return xnext

    def get_discrete_time_Koopman_Operator(self):
        # return torch.matrix_exp(self.dt * self.get_K())
        return torch.matrix_exp(self.dt * self.get_K_state())
    
    def get_discrete_time_Koopman_Operator_action(self):
        return torch.matrix_exp(self.dt * self.get_K_action())
...
class DissipativeKoopmanOperator_with_control(BaseKoopmanOperator_with_control):
    """
    ...
    def __init__(self, n, m, **kwargs):
        ...
        self.register_parameter(
            ...
            torch.nn.Parameter(
                torch.nn.init.trunc_normal_(
                    torch.zeros(self.dim, self.m), std=self.init_std
                ),
                requires_grad=True,
            ),
        )
    ...
    def get_K_state(self):
        """ Computes the dim (dim * dim) mat for state in the lifted space """
        return torch.diag(self.diagonal) + self.off_diagonal - self.off_diagonal.t()

    def get_K_action(self):
        """ Computes the dim (dim * m) mat for action to state in the lifted space"""
        return self.action_matrix

    def get_K(self):
        """ Computes the whole Koopman Operator in lifted space"""
        # 得到[A B]的拼接
        K_state = self.get_K_state()
        K_action = self.get_K_action()
        return torch.cat((K_state, K_action), dim=1)

    ### CGD
    def load_matrix(self, K_state_path, K_action_path):
        """
        从两个 .npy 文件分别加载 K_state 和 K_action，并更新 off_diagonal, diagonal 和 action_matrix。

        Args:
            K_state_path (str): 存储 K_state 的 .npy 文件路径
            K_action_path (str): 存储 K_action 的 .npy 文件路径
        """
        try:
            # 加载 K_state 和 K_action
            K_state = np.load(K_state_path)
            K_action = np.load(K_action_path)

            # 检查维度是否匹配
            if K_state.shape != (self.dim, self.dim):
                raise ValueError(f"K_state matrix shape {K_state.shape} does not match expected {(self.dim, self.dim)}")

            if K_action.shape != (self.dim, self.m):
                raise ValueError(f"K_action matrix shape {K_action.shape} does not match expected {(self.dim, self.m)}")

            # 提取 diagonal 和 off_diagonal
            diagonal_values = torch.diag(torch.tensor(K_state, dtype=torch.float32))
            off_diagonal_values = torch.tensor(K_state, dtype=torch.float32) - torch.diag(diagonal_values)
            action_matrix_values = torch.tensor(K_action, dtype=torch.float32)

            # 更新参数
            # with torch.no_grad():
            #     self.diagonal.copy_(diagonal_values)
            #     self.off_diagonal.copy_(off_diagonal_values)
            #     self.action_matrix.copy_(action_matrix_values)
            self.diagonal.copy_(diagonal_values)
            self.off_diagonal.copy_(off_diagonal_values)
            self.action_matrix.copy_(action_matrix_values)

            print(f"Koopman matrices successfully loaded from {K_state_path} and {K_action_path}")

        except Exception as e:
            print(f"Failed to load Koopman matrices: {e}")


    ### CGD
    def freeze_matrix(self):
        """ 冻结 Koopman 矩阵的参数，使其在训练中不更新 """
        for param in self.parameters():
            param.requires_grad = False
        print("Koopman matrix parameters are frozen.")
...
class IdentityDecoder(torch.nn.Module):
    ...
    def forward(self, x):
        return x[:, :self.output_size]  # 直接取前 output_size 维
...
class DLKoopmanRegressor(L.LightningModule):
    ...
    Attributes:
        input_size (int): Size of input to the encoder.     # 33
        output_size (int): Size of output from the encoder. # 33*2
    ...
    def __init__(
        ...
        self.n = n
        self.m = m
        self.dim = config_encoder["output_size"]
        ...
        ### 两个网络都是简单的前馈神经网络，都只针对于state部分，不包括action
        ### x: 21 ---> 128, u: 6 ---> 6是单独的
        self._encoder = FFNN(
        ...
        ### x: 128 ---> 21, u: 6 ---> 6是单独的
        if IDEN_DECODER:
            self._decoder = IdentityDecoder(output_size=config_decoder["output_size"])
        else:
            self._decoder = FFNN(
        ...
        # propagation in lifted space
        if mode == "Dissipative_control": # with control
            self._koopman_propagator = DissipativeKoopmanOperator_with_control(
                n=n, m=m, dim=config_encoder["output_size"], dt=dt, init_std=std_koopman
            )
        else:
    ...
    def load_encoder_weights(self, encoder_path):
        """
        从 .pth 文件中加载 encoder 的权重
        Args:
            encoder_path (str): .pth 模型文件路径
        """
        try:
            self._encoder.load_state_dict(torch.load(encoder_path, map_location=self.device))
            self._encoder.eval()  # 切换到评估模式，防止 Dropout 或 BatchNorm 影响
            print(f"Encoder weights loaded from {encoder_path}")
        except Exception as e:
            print(f"Failed to load encoder weights: {e}")

    ### CGD
    def load_koopman_matrix(self, K_state_path, K_action_path):
        """
        从 .npy 文件中加载 Koopman 矩阵
        Args:
            K_state_path (str): 存储 K_state 的 .npy 文件路径
            K_action_path (str): 存储 K_action 的 .npy 文件路径
        """
        try:
            self._koopman_propagator.load_matrix(K_state_path, K_action_path)  # 你需要在 DissipativeKoopmanOperator_with_control 里实现这个方法
            print(f"Koopman matrix loaded!")
        except Exception as e:
            print(f"Failed to load Koopman matrix: {e}")
    ...
    ### CGD
    def freeze_matrix(self):
        self._koopman_propagator.freeze_matrix()
        print("Koopman matrix parameters are frozen.")
    ...
    def forward(self, x, n=1):
        ...
        """
        if CHECK_FORWARD_FUNCTION:
            print("###### forward function in DLKoopmanRegressor")

        if useNNDMDc:
            # debug: print the device of all the variables
            # print(f"x device: {x.device}")
            state = x[:, :self.n]
            action = x[:, self.n:]
            encoded_state = self._encoder(state)
            # print(f"encoded_state device: {encoded_state.device}")
            encoded_action = action
            # print(f"encoded_action device: {encoded_action.device}")
            encoded = torch.cat((encoded_state, encoded_action), dim=1)
        else:   
            encoded = self._encoder(x)

        # debug: print size and type of n
        phi_seq = self._propagate_encoded_n_steps(encoded, n)
        decoded = self._decoder(phi_seq[:, -1, :])
        if CHECK_GRAD:
            print(f"Output in DLKR: x requires_grad: {decoded.requires_grad}, grad_fn: {decoded.grad_fn}")
    ...
    def forward_all(self, x, n):
        ...
        if CHECK_FORWARD_FUNCTION:
            print("###### forward_all function in DLKoopmanRegressor")

        encoded = self._encoder(x)
        phi_seq = self._propagate_encoded_n_steps(encoded, n)
        decoded = torch.zeros(x.size(0), n, self.encoder_input_size)
        for i in range(n):
            decoded[:, i, :] = self._decoder(phi_seq[:, i, :])
        if CHECK_GRAD:
            print(f"Output in DLKRall: x requires_grad: {decoded.requires_grad}, grad_fn: {decoded.grad_fn}")
    ...
    def _propagate_encoded_n_steps(self, encoded, n):
        ...
        Args:
            encoded (torch.Tensor): The encoded tensor of shape 
            (batch_size, encoded_size). 
            n (int): The number of steps to propagate.

        Returns:
            torch.Tensor: The propagated encoded tensor of shape 
            (batch_size, n, encoded_size).
        """
        # encoded ( batch_size, 134[state + action] )
        encoded_future_state = []
        store_action_series = encoded[:, self.dim:]

        for i in range(n):  # one step by propagator
            # predict
            encoded_state = self._koopman_propagator(encoded)
            # update

            # encoded[:, :self.dim] = encoded_state # 这是inplace操作
            # store_action_series[:-1, :] = store_action_series[1:, :]
            # store_action_series[-1, :] = 0
            # encoded[:, self.dim:] = store_action_series
            encoded = torch.cat((encoded_state, store_action_series), dim=1)
            
            # store
            encoded_future_state.append(encoded_state)
            
        return torch.stack(encoded_future_state, 1)

    def training_step(self, batch, batch_idx):  # 计算训练中每一步的loss值
        """
        ...
        """
        # unpack batch data
        x, y, ys = batch            # x(40, 27), y(40, 1, 27), ys(40)
        y_state = y[:, :, :self.n]  # only true state will be recorded
        x_state = x[:, :self.n]
        x_action = x[:, self.n:]
        batch_look_forward = ys.max()   # get the max look forward in this batch

        ### encode x for control
        encoded_state = self._encoder(x_state)
        encoded_action = x_action
        encoded_x = torch.cat((encoded_state, encoded_action),dim=1)

        ### 这里有个action信息的获取问题，有一点难搞，可能也需要MPC那样子来引入RL
        ...
        decoded_y_seq_rnn = torch.zeros(
            (x.size(0), self.look_forward, self.decoder_output_size), device=self.device
        )
        ...
        decoded_x_state = self._decoder(encoded_state)
        if REC_LOSS:
            rec_loss = torch.nn.functional.mse_loss(decoded_x_state, x_state)
        else:
            rec_loss = 0
        ...
        for i in range(batch_look_forward):
            decoded_y_seq_rec[:, i, :] = self._decoder(self._encoder(y_state[:, i, :]))
        if REC_LOSS and RNN_LOSS:
            rec_loss += self.masked_loss_metric(decoded_y_seq_rec, y, ys)
        else:
            pass

        # 总损失 = 预测损失(预测值vs真实值) + 重建损失(->encoder->decoder的重建误差)
    ...
    def configure_optimizers(self): # 配置优化器
        ...
        """
        if self.using_lbfgs:    # 使用L_BFGS优化器
...
class TensorNormalize(nn.Module):
    ...
    def forward(self, tensor: torch.Tensor):
        ...
        """
        if CHECK_FORWARD_FUNCTION:
            print("###### forward function in TensorNormalize")

        # print("########################### ############################")
        # print("tensor size = ", tensor.shape)
        # print("self.mean size = ", self.mean.shape)
        # print("self.std size = ", self.std.shape)
        # print("########################### ############################")
...
class InverseTensorNormalize(nn.Module):
    ...
    def forward(self, tensor: torch.Tensor):
        if CHECK_FORWARD_FUNCTION:
            print("###### forward function in InverseTensorNormalize")
        ...
        # 解决维度不一致
        if tensor.shape[1] < self.std.shape[0]:
            tensor_full = torch.zeros((1, self.std.shape[0]))
            tensor_full[:, :tensor.shape[1]] = tensor[:, :]
    
        # return torch.multiply(tensor, self.std) + self.mean
        return torch.multiply(tensor_full, self.std) + self.mean
...
class NNDMDcm(BaseRegressor):
    ...
    def __init__(
        ...
        self.n = n  # n state from env
        self.m = m  # n action from env
        self.mode = mode
        self.look_forward = look_forward
        ...
        self._regressor = DLKoopmanRegressor(
            n, m, mode, dt, look_forward, config_encoder, config_decoder, lbfgs, std_koopman
        )
    ...
    def fit(self, x, y=None, dt=None):  # 进行训练
        ...
        """
        # 创建Trainer对象，负责管理训练的流程
        # build trainer
        self.logger = TensorBoardLogger('tb_nndmd_logs', name='log_nndmd')
        self.trainer = L.Trainer(**self.trainer_kwargs, logger=self.logger)

        self.n_input_features_ = self.config_encoder["input_size"]

        # 使用了数据模块，负责处理输入的数据，拆分好数据用于training和validation，分4种情况来讨论
        ...
        # 训练，传入regressor和数据dm，使用的是PyTorch Lightning中核心方法
        # 该过程涉及加载数据、调用training_step方法前向传播、计算loss并使用优化器更新参数、进行验证集验证
        # trainer starts to train
        self.trainer.fit(self._regressor, self.dm)

        # 计算高维空间中的状态矩阵_state_matrix
        ...
        # self._action_matrix_ = (
        #     self._regressor._koopman_propagator.get_discrete_time_Koopman_Operator_action()
        #     .detach()
        #     .numpy()
        # )
        
        # self modified
        self._K_state_ = self._regressor._koopman_propagator.get_K_state().detach().numpy()
        self._K_action_ = self._regressor._koopman_propagator.get_K_action().detach().numpy()
        self._K_ = self._regressor._koopman_propagator.get_K().detach().numpy()

        # 计算特征量
        # [self._eigenvalues_, self._eigenvectors_] = np.linalg.eig(self._state_matrix_)
        [self._eigenvalues_, self._eigenvectors_] = np.linalg.eig(self._state_matrix_.detach().cpu().numpy())


        self._coef_ = self._state_matrix_

        ### 计算decoder的线性变换矩阵，会得到矩阵C
        if IDEN_DECODER:
            # 如果 decoder 是 IdentityDecoder，则直接构造单位矩阵的子矩阵
            latent_dim = self._regressor._encoder.layers[-1].out_features  # 获取编码后的维度
            output_size = self._regressor._decoder.output_size  # IdentityDecoder 里存了 output_size

            self._ur = np.eye(output_size, latent_dim)  # 生成 output_size × latent_dim 的单位子矩阵
            
            # 对decoder进行归一化
            if self.normalize:
                std = self.dm.inverse_transform.std
                if self._ur.shape[0] < std.shape[0]:
                    self._ur_full = np.zeros((std.shape[0], self._ur.shape[1]))
                    self._ur_full[:self._ur.shape[0], :] = self._ur[:, :]
                self._ur = np.diag(std) @ self._ur_full
        else:
            # obtain effective linear transformation
            decoder_weight_list = []

            for i in range(len(self._regressor._decoder.layers)):
                if isinstance(self._regressor._decoder.layers[i], nn.Linear):  # 只获取 Linear 层的权重
                    decoder_weight_list.append(
                        # self._regressor._decoder.layers[i].weight.detach().numpy()
                        self._regressor._decoder.layers[i].weight
                    )   # 防止decoder报错，只使用weight处理线性层

            if len(decoder_weight_list) > 1:
                # 若decoder有多层，会将多层权重矩阵相乘得到最终的C
            ...
            if self.normalize:
                ...
                ### 有维度错误
                # print("##############################")
                # print("self._ur size = ", self._ur.shape)   # (21, 128)
                # print("diag size = ", np.diag(std).shape)   # (27, 27)
                # print("std size = ", std.shape)             # 27
                # print("##############################")
        ...
        ### 有报错，权衡处理了一个没有进行normalize时的报错
        self._ur_full = self._ur

        # self._unnormalized_modes = self._ur @ self._eigenvectors_
        self._unnormalized_modes = self._ur_full @ self._eigenvectors_
    ...
    #     """
    #     # print("@@@@@@@@@@@@@@@@@ using the predict function in nndmdc @@@@@@@@@@@@@@@@@ ")
    ...
    #         else:
    #             # x is x_and_u, size 27
    #             y = self._regressor(x, n).numpy()
    #         return y

    def predict(self, x, n=1):
        if CHECK_FORWARD_FUNCTION:
            print("###### predict function in NNDMDcm")
        # self._regressor.eval()
        ...
        if self.normalize:
            y = self.dm.normalization(x)
            y = self._regressor(y, n)
            y = self.dm.inverse_transform(y).cpu().numpy()
        else:
            # y = self._regressor(x, n).cpu().numpy()
            y = self._regressor(x, n)
            if isinstance(y, torch.Tensor):
                y = y
        
        if CHECK_GRAD:
            print(f"Output in NNDMDcm: y requires_grad: {y.requires_grad}, grad_fn: {y.grad_fn}")
    ...
    @property
    def A(self):    # 升维空间中的矩阵A
        """Returns the state transition matrix A of the NNDMDc model.
    ...
    @property
    def B(self):    # 升维空间中的矩阵B，不能正确计算
        # return self._action_matrix_
        pass
    ...
    def _compute_phi(self, x_col):  # 将x映射到phi(x)，不涉及fit，只是正向的计算
        """
        ...
        """
        # correct the shape of x
        if x_col.ndim == 1:
            x_col = x_col.reshape(-1, 1)
        x = x_col.T

        # convert the x to tensor
        self._regressor.eval()
        x = self._convert_input_ndarray_to_tensor(x)
    ...
    @property
    def K_state_(self):
        return self._K_state_
    
    @property
    def K_action_(self):
        return self._K_action_
    ...
    def save_model(self, model_dir):
        """
        保存训练好的 encoder、decoder 和 Koopman 矩阵参数到文件，分开存储。
        ...
        # 保存 encoder 和 decoder 的状态字典
        encoder_path = os.path.join(model_dir, "encoder.pth")
        # decoder_path = os.path.join(model_dir, "decoder.pth")
        torch.save(self._regressor._encoder.state_dict(), encoder_path)
        # torch.save(self._regressor._decoder.state_dict(), decoder_path)
        
        # 提取并保存 K_state 和 K_action 矩阵
        koopman_matrices = {
            'K_state': self._regressor._koopman_propagator.get_K_state().detach().numpy(),
            'K_action': self._regressor._koopman_propagator.get_K_action().detach().numpy()
        }
        np.save(os.path.join(model_dir, "K_state.npy"), koopman_matrices['K_state'])
        np.save(os.path.join(model_dir, "K_action.npy"), koopman_matrices['K_action'])
    ...
    def load_encoder_weights(self, encoder_path):
        ...
        Args:
            encoder_path (str): encoder 参数文件路径。
        """
        self._regressor.load_encoder_weights(encoder_path)
        print(f"Encoder weights loaded from {encoder_path}")
    ...
    def load_koopman_matrix(self, K_state_path, K_action_path):
        ...
        Args:
            koopman_matrices (dict): 包含 K_state 和 K_action 矩阵的字典。
        """
        self._regressor.load_koopman_matrix(K_state_path, K_action_path)
        print("Koopman matrices loaded.")
        
    def freeze_encoder(self):
        """
        冻结 encoder 的参数，使其不再训练。
        """
        self._regressor.freeze_encoder()
        print("Encoder frozen.")
    
    def freeze_matrix(self):
        """
        冻结 Koopman 矩阵的参数，使其不再训练。
        """
        self._regressor.freeze_matrix()
        print("Koopman matrix frozen.")       
